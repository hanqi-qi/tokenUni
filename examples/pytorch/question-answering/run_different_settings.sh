...
do 
    ...
    do
        ...
        do
            bash run_qa_no_trainer.sh $apply_exrank $lnv $model_name>"./outlog/0702${model_name}_${apply_exrank}-${lnv}-${ifmask}_alphaN01.out" 2>&1
        done
    done
done
...
do
    bash run_qa_no_trainer.sh "None" "origin" $model_name >"./outlog/0702${model_name}_None-baseline_alphaN01.out" 2>&1
done
