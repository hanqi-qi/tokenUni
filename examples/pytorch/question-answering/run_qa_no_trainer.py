...
def parse_args():
    ...
    parser.add_argument(
        "--do_train",type=bool,default=True,help="if train"
    )
    parser.add_argument(
        "--apply_exrank",
        type=str,
...
def main():
    ...
    #TODO(yhq): 0503:Detecting last checkpoint.
    ...
    elif args.model_name_or_path:#1
    ...
    elif args.model_name_or_path:#2
    ...
    #TODO(yhq): check the customized config for model
    ...
    elif args.model_name_or_path=="albert-base-v1":
        config = AlbertConfig(
            lnv = args.lnv,
            max_seq_length = args.max_seq_length,
            spectral_norm = args.spectral_norm,
    ...
    #TODO(yhq): use eval as predict samples
    if args.do_predict:
    ...
    #TODO(yhq0514):dataset error in distill model, no token_type_ids
    # eval_dataset.set_format(type="torch", columns=["attention_mask", "input_ids", "token_type_ids"])
    #TODO(yhq0525): modify the error according to the latest gitbuh
    ...
    #         checkpoint = None
    #
    # checkpoint_file = "/mnt/Data3/hanqiyan/rank_transformers/tmp/debug_squad/{}/{}/pytorch_model.bin".format(args.model_name_or_path,args.lnv)
    # state_dict = torch.load(checkpoint_file)
    # model.load_state_dict(state_dict)
    # args.num_train_epochs = 0 #will NOT enter the training  loop
    ...
    #TODO(yhq): skip train process if do_train is False and the model.bin exists!
    # if os.path.exists(args.output_dir) and not args.do_train:
    ...
    #TODO(yhq): Modify the training process, like fetch the intermediate output
    ...
    for epoch in range(args.num_train_epochs):
        ...
        # print()
        # print("The epoch %d-th in train"%epoch)
        model.train()
    ...
    all_end_logits = []
    all_hidden_states = []
    print("begin_valiations!")
    for step, batch in enumerate(eval_dataloader):
        print("The %d -th step in eval"%step)
        with torch.no_grad():
            outputs = model(**batch)
            ...
            # if outputs.hidden_states[0].shape[0] == args.per_device_eval_batch_size:
                # all_hidden_states.append(torch.stack(outputs.hidden_states).permute(1,0,2,3))
        #TODO(yhq): add vis_tools in eval.
        # hidden_states_layers = torch.stack(outputs.hidden_states).permute(1,0,2,3)#[sample_i,i_layer,seqlen,dim]
        # vis_tools.save_matrix(hidden_states_layers,step,args,mode="eval")
    ...
    #TODO(yhq): save the intermediate output for prediction. only 
    # hidden_states_layers = torch.stack(all_hidden_states)#[all_samples,i_layer,seqlen,dim]
    # vis_tools.save_matrix(hidden_states_layers,epoch,args,mode="eval")
    # vis_tools.PlotEigen(hidden_states_layers, epoch, args,mode="eval",predict_metric=eval_metric)


    """Don't know what's wrong with the prediction, so just train and eval"""
    # Prediction
    if args.do_predict:
        all_start_logits = []
        all_end_logits = []
        #TODO(yhq): visualize all the predict samples, but the predictdata are on cpu,so...
        predict_dataloader = eval_dataloader
        all_hidden_states = []
        for step, batch in enumerate(predict_dataloader):
            with torch.no_grad():
                #TODO(yhq): why the data on cpu.
        ...
        # #TODO(yhq): save the intermediate output for prediction 
        # hidden_states_layers = np.stack(outputs.hidden_states)#[all_samples,i_layer,seqlen,dim]
        # vis_tools.save_matrix(hidden_states_layers,epoch,args,mode="eval")
        # vis_tools.PlotEigen(input_tensor, epoch, args,mode="eval",predict_metric=predict_metric)
