...
class ExRank_gx(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.LayerNorm = NormFuncs(config.hidden_dim, args=config)
...
class FFN(nn.Module):
    ...
    def forward(self, input,i_layer):
        return apply_chunking_to_forward(self.ff_chunk,i_layer,self.chunk_size_feed_forward, self.seq_len_dim, input)

    def ff_chunk(self, input,i_layer):
        x = self.lin1(input)
        x = self.activation(x)
        x = self.lin2(x)
        x = self.dropout(x)
...
class TransformerBlock(nn.Module):
    """similar to bertlayer,but without the bertoutput layer."""
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, x, attn_mask=None, head_mask=None, output_attentions=False,i_layer=0):
        """
        Parameters:
