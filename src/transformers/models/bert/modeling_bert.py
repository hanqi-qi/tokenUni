...
#TODO(yhq): define the Exrank_layer
class ExRank_gx(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.LayerNorm = NormFuncs(config.hidden_size, args=config)
...
#TODO(yhq): output transformation for each attention.
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        all_old_hidden_states = () if output_hidden_states else None
        ...
        for i, layer_module in enumerate(self.layer):
            if output_hidden_states:
                ...
                all_old_hidden_states = all_old_hidden_states + (hidden_states,)
            ...
            #TODO(yhq0512):
                ...
                # old_hidden_states = self.exrank_layer(old_hidden_states)
            if use_cache:
...
#TODO(yhq): different heads for fine-tuning downstream tasks
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        sequence_output = encoder_outputs[0]#encoder_outputs["last_hidden_state"]
        pooled_output = self.pooler(sequence_output) if self.pooler is not None else None

        #TODO(yhq): obtain the hidden_states for all the layers
        # hidden_states_layers=encoder_outputs_layers[-1]
        # all_outputs = torch.stack(hidden_states_layers)
        #TODO(yhq): 0504, the default last layer output is the pooling results. use the deault encoder_outputs.hidden_states as 
        # all_outputs = torch.stack(encoder_outputs["hidden_states"])#len=13, including the embedding->[13,bs,seqlen,dim]
...
class BertForTokenClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        if not logits.shape[0] == self.config.batch_size_yhq:
        ...
        if labels is not None:
            ...
            if attention_mask is not None:
                ...
                #TODO(yhq): error here
                if not active_logits.shape[0] == active_labels.shape[0]:
                    print(active_logits.shape) #[1024,14]
                    print(active_labels.shape)#[1024] when it is normal
                    print(logits.shape[0])
        ...
        if not return_dict:
            output = (logits,) + outputs[2:]
            return ((loss,) + output) if loss is not None else output
        # print("last module output_hidden_states")
        # print(outputs.hidden_states[0][0,:5,:3])
        # print(outputs.hidden_states[11][0,:5,:3])
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def forward(
        ...
        sequence_output = outputs[0]#last layer output
        ...
        #TODO(yhq): define customized way for various downstream tasks. define final output
